...
cd ${USER_HOME}
# Check if the tx-pi-setup.sh script does not exist in the current directory
if [ ! -f tx-pi-setup.sh ]
then
  # Download the tx-pi-setup.sh script from the specified URL if it doesn't exist
  wget https://raw.githubusercontent.com/ftCommunity/tx-pi/refs/heads/develop/setup/tx-pi-setup.sh
fi
# Change the ownership of the tx-pi-setup.sh script to the current user
chown "${USER_NAME}": tx-pi-setup.sh
# Switch to the current user and execute the tx-pi-setup.sh script with debug output enabled
su --login --command "bash -x tx-pi-setup.sh" "${USER_NAME}"
